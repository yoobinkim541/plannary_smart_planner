import {
    getEl, showToast, escapeHtml, appIconSvg,
...
// getEl, showToast, escapeHtml, appIconSvg and the set* DOM helpers moved to src/dom.js.
...
async function saveTaskEditDialog() {
    ...
    const payload = {
        ...
        tzOffsetMinutes: getEl('task-edit-due-time')?.value ? -(new Date().getTimezoneOffset()) : null,
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (getEl('add-btn')) getEl('add-btn').onclick = async () => {
        ...
        const payload = {
            ...
            tzOffsetMinutes: timeInput.value ? -(new Date().getTimezoneOffset()) : null,
